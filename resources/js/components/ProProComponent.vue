<template>
  <div>
    ...
    <hr>
    <h3 class="form-control-label text-muted">Lista de problemas de produccion</h3>
    <ul class="list-group">
         <li class="list-group-item"
            v-for="(item, index) in pros_pro" :key="index" >
...
export default {
  ...
  created(){
    axios.get('/propro').then(res=>{
      this.pros_pro = res.data;
    });
  },
  methods:{
    agregar(){
      ...
      axios.post('/propro', proNueva)
        .then((res) =>{
          const proServidor = res.data;
          this.pros_pro.push(proServidor);
        })
    },
    ...
    editarPro(pro){
      ...
      axios.put(`/propro/${pro.id}`, params)
        .then(res=>{
          this.modoEditar = false;
          const index = this.pros_pro.findIndex(item => item.id === pro.id);
          this.pros_pro[index] = res.data;
        })
    },
    eliminarPro(pro, index){
      const confirmacion = confirm(`Eliminar problema ${pro.descripcion}`);
      if(confirmacion){
        axios.delete(`/propro/${pro.id}`)
          .then(()=>{
            this.pros_pro.splice(index, 1);
          })
      }
    },
    cancelarEdicion(){
